...
export default function ImportTestCases() {
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1000, margin: '0 auto' }}>
      <button onClick={() => navigate('/test-cases')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280', fontSize: 14, padding: 0, marginBottom: 20 }}>
      ...
      {result ? (
        <div style={card}>
          ...
          {result.skipped.length > 0 ? (
            ...
            <p style={{ fontSize: 14, color: '#6b7280', margin: 0 }}>No rows were skipped.</p>
          )}
        ...
        <>
          ...
          <div style={{ ...card, marginBottom: preview ? 20 : 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              ...
              {fileName && <span style={{ fontSize: 13, color: '#6b7280' }}>{fileName}</span>}
              {phase === 'parsing' && <span style={{ fontSize: 13, color: '#9ca3af', fontStyle: 'italic' }}>Parsing…</span>}
            ...
            <div style={card}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
                <div style={{ display: 'flex', gap: 16, fontSize: 14, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <span style={{ color: '#9ca3af' }}>· {preview.summary.total} total</span>
                </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
                  ...
                  <tbody>
                    {preview.rows.map((r, i) => (
                      <tr key={r.rowNumber} style={{ background: i % 2 ? '#fafafa' : '#fff', borderBottom: '1px solid #f3f4f6', opacity: r.valid ? 1 : 0.85 }}>
                        <td style={{ ...td, color: '#9ca3af' }}>{r.rowNumber}</td>
                        <td style={td}>
                          {r.valid
                        ...
                        <td style={{ ...td, fontWeight: 500 }}>
                          {r.mapped.title || <span style={{ color: '#d1d5db' }}>—</span>}
                        ...
                        <td style={{ ...td, color: '#6b7280' }}>{r.mapped.steps.length || <span style={{ color: '#d1d5db' }}>0</span>}</td>
...
const card = { background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: 22 };
const code = { background: '#f3f4f6', padding: '1px 6px', borderRadius: 4, fontSize: 13, color: '#374151' };
const th = { padding: '10px 14px', textAlign: 'left', fontWeight: 600, fontSize: 12.5, color: '#374151' };
...
const secondaryBtn = { background: '#fff', color: '#374151', border: '1px solid #d1d5db', padding: '8px 16px', borderRadius: 6, cursor: 'pointer', fontWeight: 600, fontSize: 14 };
